...
Проект реализует вариант 14: фитнес-трекер. Основные сущности системы: пользователь, упражнение и тренировка.
...
Поля `profile` и `goals` сделаны embedded, потому что они относятся только к одному пользователю и обычно читаются вместе с ним. Отдельная коллекция для этих данных не нужна.
...
Упражнения вынесены в отдельную коллекцию, потому что они переиспользуются в разных тренировках. В тренировках хранится reference на `exercise_id`, а также краткий снимок названия упражнения, чтобы историю тренировки было удобно читать без лишних запросов.
...
Тренировка хранит reference на пользователя через `user_id`. Список упражнений внутри тренировки сделан embedded-массивом, потому что упражнения тренировки почти всегда нужны вместе с самой тренировкой: при получении истории тренировок пользователь ожидает сразу увидеть состав каждой тренировки.
...
- `workouts.exercises` встроен в тренировку, потому что состав тренировки читается вместе с тренировкой и содержит параметры выполнения: подходы, повторения, длительность и сожженные калории.
...
Основные endpoint-ы PostgreSQL из предыдущих работ сохранены. Для демонстрации подключения API к MongoDB добавлены endpoint-ы:
